    ...
    <jet-dialog-modal :show="managingPermissionsFor" @close="managingPermissionsFor = null">
      ...
      <template #content>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="permission in availablePermissions">
            <label class="flex items-center">
              <input v-model="updateApiTokenForm.permissions" :value="permission" class="form-checkbox" type="checkbox">
              <span class="ml-2 text-sm text-gray-600">{{ permission }}</span>
            </label>
          </div>
        </div>
      ...
      <template #footer>
        ...
        <jet-button :class="{ 'opacity-25': updateApiTokenForm.processing }" :disabled="updateApiTokenForm.processing"
                    class="ml-2" @click.native="updateApiToken">
          Save
        </jet-button>
      </template>
    </jet-dialog-modal>
    ...
    <jet-confirmation-modal :show="apiTokenBeingDeleted" @close="apiTokenBeingDeleted = null">
      ...
      <template #footer>
        ...
        <jet-danger-button :class="{ 'opacity-25': deleteApiTokenForm.processing }"
                           :disabled="deleteApiTokenForm.processing" class="ml-2" @click.native="deleteApiToken">
          Delete
        </jet-danger-button>
      </template>
